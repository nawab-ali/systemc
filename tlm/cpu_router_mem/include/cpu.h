...
SC_MODULE(Cpu) {
    ...
    void comm_transport_interface(tlm::tlm_command & cmd, int &address, sc_time &delay) {
        ...
        if (trans->is_response_error()) {
            char txt[100];
            snprintf(txt, 100, "Error from b_transport, response status = %s", trans->get_response_string().c_str());
        ...
        cout << "trans = {" << (cmd ? 'W' : 'R') << ", " << hex << address << "}, data = " << hex << data << " at time "
    ...
    void comm_dmi(tlm::tlm_command & cmd, int &address) {
        ...
        cout << "dmi   = {" << (cmd ? 'W' : 'R') << ", " << hex << address << "}, data = " << hex << data << " at time "
             << sc_time_stamp() << endl;
    }
